...
export function HeaderLogo({ href = "/", className, onClick, ...props }: HeaderLogoProps) {
  ...
  return (
    <Link
      href={href}
      className={cn(
        "group relative inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-brand-from via-brand-via to-brand-to shadow-md transition-all duration-300 ease-out hover:scale-110 hover:shadow-brand-via/40 hover:shadow-lg",
        className
      )}
      aria-label="Home"
      onClick={(e) => {
    ...
    >
      ...
      <span className="absolute inset-0 bg-gradient-to-br from-brand-to via-brand-from to-brand-via opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      {/* Shimmer effect */}
